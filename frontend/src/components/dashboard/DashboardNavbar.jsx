...
import { Shield, MapPin, User, LogOut, Settings, Bell } from 'lucide-react';
...
const DashboardNavbar = () => {
  ...
  return (
    <header className="h-16 border-b border-gray-100 bg-white px-6 flex items-center justify-between sticky top-0 z-40 shadow-sm">
      ...
      {telemetry && (
        <div className="hidden md:flex items-center space-x-4 bg-gray-50 py-1.5 px-3 rounded-lg border border-gray-100">
      ...
      <div className="flex items-center space-x-4">
        ...
        {riskAssessment && riskAssessment.overallRisk > 40 && (
          <button className="relative p-2 rounded-full hover:bg-gray-100 transition-colors text-lifelink-amber animate-bounce">
            <Bell className="w-5 h-5" />
            <span className="absolute top-1 right-1 w-2.5 h-2.5 bg-lifelink-red rounded-full ring-2 ring-white" />
          </button>
        )}
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center space-x-3 p-1.5 rounded-lg hover:bg-gray-50 transition-colors border border-transparent hover:border-gray-100"
          >
            {user && user.profilePicture ? (
              <img
                src={user.profilePicture}
                alt={user.name}
                className="w-8 h-8 rounded-full object-cover ring-2 ring-lifelink-green/20"
              />
            ) : (
              <div className="w-8 h-8 rounded-full bg-lifelink-green text-white flex items-center justify-center font-bold text-sm ring-2 ring-lifelink-green/20">
                {getProfileInitials()}
              </div>
            )}
